...
def format(route):
    destination_ip, from_port, to_port, connection_ip, connection_port = tuple(route.strip().split(' '))
    return (destination_ip, int(from_port), int(to_port), connection_ip, int(connection_port), 0)
